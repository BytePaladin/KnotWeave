...
> An interactive Android simulation engine and Resource Allocation Graph (RAG) visualizer for analyzing concurrency, Banker's Algorithm safety, and system deadlocks in real time.
...
- **Real-Time Deadlock Detection & Safety Sequences:** Instant evaluation of cycle dependencies, Banker's safe states, and valid execution sequences.
- **Visual Deadlock & Safety Highlighting:** Deadlocked processes and blocked edges are dynamically highlighted on canvas.
- **Customizable Resource Instances & Max Needs:** Real-time configuration of single-instance or multi-instance resource capacities and per-process max demands.
...
* **Max Matrix ($M \in \mathbb{Z}^{N \times M}$):** Maximum demand declared for resource $R_j$ by process $P_i$.
* **Need Matrix ($N \in \mathbb{Z}^{N \times M}$):** Remaining resource requirement: $N[i][j] = \max(Q[i][j], M[i][j] - A[i][j])$.
...
Deadlock detection and Banker's safety are computed by simulating resource reclamation over a working vector:
...
└── sdk/                               # Core KnotWeave Library Module
    └── src/main/java/com/knotweave/sdk/
        ├── BankersAlgorithm.java      # Pure Java 2D Matrix Math & Deadlock Engine
...
- **`DeadlockCanvasView.java`:** Hardware-accelerated custom `View` that renders processes, resources, dynamic Bezier cables, and touch interaction ports.
- **`KnotWeaveManager.java`:** Orchestrates canvas touch events, node property modifications, safe sequence updates, and evaluation triggers.
...
1. Clone the repository:
   ```bash
   git clone https://github.com/BytePaladin/KnotWeave-temp.git
   ```
...
3. **Inspect Properties & Max Need:** Tap any node to edit its label, instance capacity, or per-process max need values.
4. **Observe Real-Time Analysis:** The status banner immediately indicates `SAFE STATE` (displaying the valid safe execution sequence) or `DEADLOCK DETECTED` (highlighting the deadlocked processes in red).
